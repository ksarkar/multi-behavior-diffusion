...
to setup-preferential-attachment-network
  ...
  let first-node make-node        ;; first node, unattached
  let second-node make-node        ;; second node
  make-edge first-node second-node
  
  ;; now add rest of the nodes
  repeat number-of-nodes - 2 [
    let new-node make-node
    repeat number-of-edges [
      make-edge new-node (find-partner new-node) 
    ]
...
;; used for creating a new edge
to make-edge [new-node old-node]
  ask new-node [
...
;; Finds partner for node
to-report find-partner [node]
  let rest turtles
  ask node [
    set rest other rest
  ]
  
  let total random-float sum [count link-neighbors] of rest
  let partner nobody
  ask rest
  [
...
<experiments>
  ...
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="number-of-edges">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="num-sim-for-spread-based-seed-selection">
      <value value="200"/>
  ...
  <experiment name="1-1-th-r-nav" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="100"/>
    ...
    <steppedValueSet variable="rand-seed-network" first="2164" step="1" last="3163"/>
    <enumeratedValueSet variable="max-step">
      <value value="50"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="num-behaviors">
